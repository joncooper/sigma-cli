...
def list_workspaces(
    ...
    try:
        ...
        response = client.get("/v2/workspaces", params=params)

        if table and "entries" in response:
            entries = response["entries"]
            columns = ["workspaceId", "name", "type", "createdBy"]
            print_table(entries, columns=columns, title="Workspaces")
        else:
...
def get_workspace(
    ...
    try:
        client = get_client(client_id, secret, base_url)
        response = client.get(f"/v2/workspaces/{workspace_id}")
        print_json(response, pretty=pretty, highlight=True)
    except Exception as e:
        print_error(str(e))
        raise typer.Exit(1)
...
def create_workspace(
    ...
    try:
        ...
        response = client.post("/v2/workspaces", json_data=json_data)
        print_json(response, pretty=pretty, highlight=True)
...
def update_workspace(
    ...
    try:
        ...
        response = client.patch(f"/v2/workspaces/{workspace_id}", json_data=json_data)
        print_json(response, pretty=pretty, highlight=True)
...
def delete_workspace(
    ...
    try:
        client = get_client(client_id, secret, base_url)
        client.delete(f"/v2/workspaces/{workspace_id}")
...
def get_workspace_members(
    ...
    """Get members with access to a workspace."""
    try:
        client = get_client(client_id, secret, base_url)
        response = client.get(f"/v2/workspaces/{workspace_id}/members")
        print_json(response, pretty=pretty, highlight=True)
    except Exception as e:
        print_error(str(e))
        raise typer.Exit(1)
