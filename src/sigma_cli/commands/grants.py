...
def list_grants(
    ...
    try:
        ...
        response = client.get("/v2/grants", params=params)

        if table and "entries" in response:
            entries = response["entries"]
            columns = ["grantId", "name", "permission", "grantee", "resource"]
            print_table(entries, columns=columns, title="Grants")
        else:
...
def get_grant(
    ...
    try:
        client = get_client(client_id, secret, base_url)
        response = client.get(f"/v2/grants/{grant_id}")
        print_json(response, pretty=pretty, highlight=True)
    except Exception as e:
        print_error(str(e))
        raise typer.Exit(1)
...
def create_grant(
    ...
    try:
        ...
        response = client.post("/v2/grants", json_data=json_data)
        print_json(response, pretty=pretty, highlight=True)
...
def update_grant(
    ...
    try:
        ...
        response = client.patch(f"/v2/grants/{grant_id}", json_data=json_data)
        print_json(response, pretty=pretty, highlight=True)
...
def delete_grant(
    ...
    try:
        client = get_client(client_id, secret, base_url)
        client.delete(f"/v2/grants/{grant_id}")
